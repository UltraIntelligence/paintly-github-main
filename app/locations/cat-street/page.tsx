...
// Mock location data for Cat Street
const location = {
  id: 3,
  name: { japanese: "アートバーキャットストリート", english: "Artbar Cat Street" },
  photo: "/placeholder.svg?height=240&width=320",
  address: {
    japanese: "〒150-0001 東京都渋谷区神宮前 4-28-4",
    english: "4-28-4 Jingumae, Shibuya, Tokyo 150-0001",
  },
  access: {
    japanese: "表参道駅から徒歩7分 | 明治神宮前駅から徒歩5分",
    english: "7-minute walk from Omotesando Station | 5-minute walk from Meiji-jingumae Station",
  },
  status: "active",
  type: "standard",
  capacity: 28,
  features: ["Street Art Focus", "Youth Programs", "Wheelchair Access"],
  openingHours: "12:00 - 21:00",
  phone: "+81 3-3456-7890",
  website: "artbar.co.jp/cat-street",
}
...
const getLocationData = (locationId: number) => ({
  ...
  yesterdayRevenue: 41500,
  eventsToday: 3,
  ticketsSold: 35,
  ticketsRemaining: 12,
  activeInstructors: 2,
  standbyInstructors: 1,
  weeklyRevenue: 198000,
  weeklyChange: 15,
})
...
const todaysEvents = [
  {
    id: 1,
    title: { japanese: "ストリートアート入門", english: "Street Art Basics" },
    time: "14:00",
    booked: 12,
    capacity: 15,
    revenue: 13200,
  ...
  {
    id: 2,
    title: { japanese: "ポップアート体験", english: "Pop Art Experience" },
    time: "16:30",
    booked: 14,
    capacity: 16,
    revenue: 15400,
  ...
  {
    id: 3,
    title: { japanese: "グラフィティワークショップ", english: "Graffiti Workshop" },
    time: "19:00",
    booked: 9,
    capacity: 16,
    revenue: 9600,
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Button
            ...
            onClick={() => router.push("/schedule?location=cat-street")}
          >
          ...
          <Button
            ...
            onClick={() =>
              window.open(
                "mailto:kenta@artbar.co.jp,mika@artbar.co.jp,ryo@artbar.co.jp?subject=Today's Schedule - Cat Street&body=Hi team,%0A%0ARegarding today's classes at Cat Street location...",
                "_blank",
              )
            }
          >
          ...
          <Button
            ...
            onClick={() => {
              const csvData =
                "Event,Time,Instructor,Booked,Capacity,Revenue\nStreet Art Basics,14:00,Kenta,12,15,¥13200\nPop Art Experience,16:30,Mika,14,16,¥15400\nGraffiti Workshop,19:00,Ryo,9,16,¥9600"
              const blob = new Blob([csvData], { type: "text/csv" })
              const url = window.URL.createObjectURL(blob)
              const a = document.createElement("a")
              a.href = url
              a.download = `cat-street-events-${new Date().toISOString().split("T")[0]}.csv`
              a.click()
          ...
          <Button
            variant="outline"
            className="h-auto flex-col py-6 px-4"
            onClick={() => router.push("/schedule?filter=cat-street")}
          >
            <Calendar className="h-6 w-6 mb-2" />
