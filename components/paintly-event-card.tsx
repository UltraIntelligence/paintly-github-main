...
interface PaintlyEventCardProps {
  ...
  actionText?: string
  image?: string
  onViewDetails?: (eventData: any) => void // Add this line
}
              ...
              {(onViewDetails || onAction) && (
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    if (onViewDetails) {
                      // Transform event data for consistency
                      const eventData = {
                        id: Math.random(), // Generate or use actual ID
                        title: title,
                        titleEn: titleJp,
                        instructor: instructor?.toLowerCase().replace(" ", "") || "yuki",
                        participants: participants,
                        status: status || "Scheduled",
                        day: 0, // Calculate from date
                        startHour: 8, // Parse from time
                        duration: 2, // Calculate from time
                        location: location.toLowerCase().replace("artbar ", "").replace(" ", ""),
                      }
